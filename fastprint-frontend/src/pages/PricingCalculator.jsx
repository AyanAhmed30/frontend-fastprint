...
const OPTIONS = {
  ...
  exteriorColor: [
    { id: 'black', name: 'Black', price: 5.00, bg: 'bg-black', image: 'https://tse1.mm.bing.net/th/id/OIP.eDT_6SV4EMT_EIvLNI1jZgHaFj?pid=Api&P=0&h=220' },
    { id: 'brown', name: 'Brown', price: 3.00, bg: 'bg-amber-800', image: 'https://tse1.mm.bing.net/th/id/OIP.2MCU2S3LitdZbInyvu_4-QHaE8?pid=Api&P=0&h=220' },
    { id: 'maroon', name: 'Maroon', price: 5.00, bg: 'bg-red-800', image: 'https://tse4.mm.bing.net/th/id/OIP.m096lDro6ncKKg4DwPmUUwHaEo?pid=Api&P=0&h=220' },
    { id: 'dark_blue', name: 'Dark Blue', price: 5.00, bg: 'bg-blue-900', image: 'https://tse4.mm.bing.net/th/id/OIP.bdBwPN-71Sz4S2yShDWCaAHaEK?pid=Api&P=0&h=220' }
  ],
  foilStamping: [

    { id: 'golden', name: 'Golden', price: 10.00, bg: 'bg-yellow-400', image: 'https://tse4.mm.bing.net/th/id/OIP.yN-LJTTQxlR-ZWO2YN_CIQHaHa?pid=Api&P=0&h=220' },
    { id: 'silver', name: 'Silver', price: 15.00, bg: 'bg-gray-300', image: 'https://i.pinimg.com/736x/fd/ac/b0/fdacb002da9862d86bc871f7ee160383.jpg' }
  ],
  screenStamping: [

    { id: 'golden', name: 'Golden', price: 10.00, bg: 'bg-yellow-400', image: 'https://www.48hrbooks.com/asset/index/213ba005-4db4-4ecd-a4cd-01f77b04994b' },
    { id: 'silver', name: 'Silver', price: 15.00, bg: 'bg-gray-300', image: 'https://smartpressblog.imgix.net/wp-content/uploads/2022/08/foil-perfect-bound-booklets_silver_02.png?auto=format%2Ccompress&ixlib=php-3.3.0&s=15005f4e66b0f086cf9041bef5b826cf&w=900' }
  ],
  cornerProtector: [
...
const OptionCard = ({ item, fieldName, fieldValue, onSelect, accessible = true, hasImage = true }) => {
  const isSelected = fieldValue === item.id;
  const opacity = accessible ? 1 : 0.3;

  return (
    <label className={`flex flex-col items-center cursor-pointer relative w-20 sm:w-24 ${!accessible ? 'cursor-not-allowed' : ''}`} style={{ opacity }}>
      <div className="relative w-full">
        ...
        {hasImage ? (
          <div className="w-full h-16 bg-gray-200 mb-2 mt-3 rounded overflow-hidden flex items-center justify-center">
            {item.image ? (
              <img
                ...
                onError={(e) => {
                  e.target.src = 'https://images.unsplash.com/photo-1544947950-fa07a98d237f?ixlib=rb-4.0.3&auto=format&fit=crop&w=200&q=80';
          ...
          <div className={`w-12 h-12 rounded border-2 mb-2 mx-auto mt-3 ${item.bg || 'bg-gray-200'}`}></div>
        )}
      </div>
...
const Section = ({ title, children }) => (
  <>
    <h2 className="text-lg sm:text-xl lg:text-2xl font-bold mb-2 text-[#2A428C]">{title}</h2>
    <div className="w-full h-0.5 bg-gray-200 mb-4 sm:mb-6"></div>
    {children}
  </>
);

const OptionSection = ({ title, options, fieldName, fieldValue, onSelect, accessible = true, hasImage = true }) => (
  <>
...
const ThesisPricingCalculator = () => {
  ...
  // Step accessibility
  const fieldOrder = ['bookSize', 'pageCount', 'binding_id', 'spine_id', 'exterior_color_id', 'foil_stamping_id', 'screen_stamping_id', 'corner_protector_id', 'interior_color_id', 'paper_type_id'];
  ...
  const calculatePrice = useMemo(() => {
    const requiredFields = ['bookSize', 'pageCount', 'binding_id', 'spine_id', 'exterior_color_id', 'foil_stamping_id', 'screen_stamping_id', 'corner_protector_id', 'interior_color_id', 'paper_type_id'];

    if (!requiredFields.every(field => form[field] !== '') || form.quantity <= 0) {
      return null;
    }

    // Find selected options
    const findOption = (type, id) => OPTIONS[type]?.find(opt => opt.id === id) || {};
    ...
    // Calculate costs
    const basePrice = [binding, spine, exteriorColor, foilStamping, screenStamping, cornerProtector]
      .reduce((sum, opt) => sum + (opt.price || 0), 0);

    const pageBasedCost = ((interiorColor.pricePerPage || 0) + (paperType.pricePerPage || 0)) * parseInt(form.pageCount);
    ...
    <>
      ...
      <div className="w-full min-h-screen px-4 sm:px-6 lg:px-8 py-8 flex flex-col xl:flex-row gap-6 xl:gap-8 bg-gray-50">
        ...
        <div className="w-full xl:w-3/5 bg-gradient-to-br from-blue-50 to-blue-100 p-4 sm:p-6 lg:p-8 rounded-2xl">
          <h2 className="text-2xl sm:text-3xl font-bold mb-6 text-[#2A428C]">Thesis Binding Calculator</h2>
  ...
  <div className="flex flex-col sm:flex-row gap-4">
    ...
    <div className="w-full sm:w-1/2 flex flex-col">
      ...
      <Input
        ...
        className="rounded-lg shadow-sm bg-white text-black disabled:bg-white disabled:opacity-100 disabled:filter-none"
      />
    </div>
  </div>
</div>
          ...
          <div className="flex flex-col gap-4 p-4 bg-gradient-to-r from-[#016AB3] via-[#0096CD] to-[#00aedc] rounded-2xl">
            <h3 className="text-white text-lg font-semibold">Quantity and Shipping Estimate</h3>
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <div className="space-y-1 text-xs text-gray-600">
                {DISCOUNT_TIERS.map((tier, i) => (
                  <div key={i} className="flex justify-between">
                    <span>{tier.min}{i < DISCOUNT_TIERS.length - 1 ? `-${DISCOUNT_TIERS[i + 1]?.min - 1}` : '+'} books:</span>
                    <span className={form.quantity >= tier.min ? 'font-semibold text-green-600' : ''}>{tier.percent}% off</span>
                  </div>
                ))}
              </div>
            </div>
