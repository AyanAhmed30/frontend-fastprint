...
// Constants
const BOOK_SIZES = ['Pocket Book (4.25 x 6.875 in / 108 x 175 mm)', 'Novella (5 x 8 in / 127 x 203 mm)', 'Digest (5.5 x 8.5 in / 140 x 216 mm)', 'A5 (5.83 x 8.27 in / 148 x 210 mm)', 'US Trade (6 x 9 in / 152 x 229 mm)', 'Royal (6.14 x 9.21 in / 156 x 234 mm)', 'Executive (7 x 10 in / 178 x 254 mm)', 'Crown Quarto (7.44 x 9.68 in / 189 x 246 mm)', 'Small Square (7.5 x 7.5 in / 190 x 190 mm)', 'A4 (8.27 x 11.69 in / 210 x 297 mm)', 'Square (8.5 x 8.5 in / 216 x 216 mm)', 'US Letter (8.5 x 11 in / 216 x 279 mm)', 'Small Landscape (9 x 7 in / 229 x 178 mm)', 'US Letter Landscape (11 x 8.5 in / 279 x 216 mm)', 'A4 Landscape (11.69 x 8.27 in / 297 x 210 mm)'];
...
const DISCOUNTS = [{ min: 1000, percent: 15 }, { min: 500, percent: 10 }, { min: 100, percent: 5 }];
...
const calculatePrice = (formData) => {
  ...
  const unitPrice = (sizePricing.binding[binding_id] || 0) + ((sizePricing.interiorColor[interior_color_id] || 0) * page_count) + ((sizePricing.paperType[paper_type_id] || 0) * page_count) + (sizePricing.coverFinish[cover_finish_id] || 0);
  const totalPrice = unitPrice * quantity;
  const discount = DISCOUNTS.find(d => quantity >= d.min);
  const discountAmount = discount ? totalPrice * (discount.percent / 100) : 0;
  return { unitPrice: unitPrice.toFixed(2), totalPrice: totalPrice.toFixed(2), discount: discountAmount.toFixed(2), discountPercent: discount?.percent || 0, finalPrice: (totalPrice - discountAmount).toFixed(2) };
};

// Reusable Components
const FormInput = ({ type = 'text', name, value, onChange, placeholder, className = "", disabled = false, ...props }) => (
  <input type={type} name={name} value={value} onChange={onChange} placeholder={placeholder} className={`w-full border px-3 py-2 rounded ${className}`} disabled={disabled} required {...props} />
);

const FormSelect = ({ name, value, onChange, options, placeholder, className = "" }) => (
  <select name={name} value={value} onChange={onChange} className={`w-full border px-3 py-2 rounded ${className}`} required>
  ...
  return (
    <label className={`flex flex-col items-center cursor-pointer relative w-20 sm:w-24 ${!canSelect ? 'cursor-not-allowed' : ''}`} style={{ opacity }}>
      <div className="relative w-full">
        <input type="radio" name={fieldName} value={value} checked={fieldValue === value} onChange={() => canSelect && onSelect(value)} disabled={!canSelect} className="absolute top-1 left-1 z-10 w-3 h-3" />
...
const SectionTitle = ({ children }) => (<><h2 className="text-lg sm:text-xl lg:text-2xl font-bold mb-2 text-[#2A428C]">{children}</h2><div className="w-full h-0.5 bg-gray-200 mb-4 sm:mb-6"></div></>);

const OptionSection = ({ title, options, fieldName, fieldValue, onSelect, isAvailable, stepAccessible, hasDbName = false }) => (<><h3 className="text-base sm:text-lg font-semibold mb-3 sm:mb-4 text-[#2A428C]">{title}</h3><div className="flex gap-3 sm:gap-4 lg:gap-6 mb-8 sm:mb-10 flex-wrap">{options.map((item, idx) => <OptionCard key={idx} item={item} fieldName={fieldName} fieldValue={fieldValue} onSelect={onSelect} isAvailable={typeof isAvailable === 'function' ? isAvailable(item) : isAvailable} stepAccessible={stepAccessible} hasDbName={hasDbName} />)}</div></>);
...
const PrintBookCalculator = () => {
  const [form, setForm] = useState({ bookSize: '', page_count: '', binding_id: '', interior_color_id: '', paper_type_id: '', cover_finish_id: '', quantity: 1 });
  ...
  const stepAccessibility = useMemo(() => ({ bookSize: true, pageCount: form.bookSize !== '', binding: form.bookSize !== '' && form.page_count !== '', interiorColor: form.bookSize !== '' && form.page_count !== '' && form.binding_id !== '', paperType: form.bookSize !== '' && form.page_count !== '' && form.binding_id !== '' && form.interior_color_id !== '', coverFinish: form.bookSize !== '' && form.page_count !== '' && form.binding_id !== '' && form.interior_color_id !== '' && form.paper_type_id !== '' }), [form]);
  const bindingOptions = useMemo(() => ({ paperback: Object.entries(BINDING_RULES).filter(([, rules]) => rules.type === 'paperback').map(([name, rules]) => ({ name, img: rules.img })), hardcover: Object.entries(BINDING_RULES).filter(([, rules]) => rules.type === 'hardcover').map(([name, rules]) => ({ name, img: rules.img })) }), []);
  ...
  const handleChange = (e) => {
    ...
    const resetFields = { bookSize: ['page_count', 'binding_id', 'interior_color_id', 'paper_type_id', 'cover_finish_id'], page_count: ['binding_id', 'interior_color_id', 'paper_type_id', 'cover_finish_id'], binding_id: ['interior_color_id', 'paper_type_id', 'cover_finish_id'], interior_color_id: ['paper_type_id', 'cover_finish_id'], paper_type_id: ['cover_finish_id'] };
    setForm(prev => {
  ...
  const handleBindingSelect = (value) => { setForm(prev => ({ ...prev, binding_id: value, interior_color_id: '', paper_type_id: '', cover_finish_id: '' })); setResult(null); };
  ...
  useEffect(() => { if (form.binding_id && !availableBindings.includes(form.binding_id)) setForm(prev => ({ ...prev, binding_id: '', interior_color_id: '', paper_type_id: '', cover_finish_id: '' })); }, [availableBindings, form.binding_id]);
      ...
      <div className="w-full min-h-screen px-4 sm:px-6 lg:px-8 py-8 flex flex-col xl:flex-row gap-6 xl:gap-8">
        <div className="w-full xl:w-3/5 bg-gradient-to-br from-blue-50 to-blue-100 p-4 sm:p-6 lg:p-8 rounded-2xl">
          ...
          <div className="flex flex-col gap-4 p-4 mb-8 bg-gradient-to-r from-[#016AB3] via-[#0096CD] to-[#00AEDC] rounded-2xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="w-full sm:w-1/2 flex flex-col">
                <div className="h-5 mb-1"><p className="text-xs text-white opacity-90">Book Size</p></div>
              ...
              <div className="w-full sm:w-1/2 flex flex-col">
                <div className="h-5 mb-1"><p className="text-xs text-white opacity-90">{form.bookSize ? 'MIN-MAX: 2 - 800' : 'Select book size first'}</p></div>
                <FormInput name="page_count" value={form.page_count} onChange={handleChange} type="number" placeholder="Enter Page Count" min="2" max="800" className="h-12 bg-white text-black rounded-md" disabled={!stepAccessibility.pageCount} />
              </div>
            </div>
          </div>
          <SectionTitle>Binding Types</SectionTitle>
          <OptionSection title="Paperback Options" options={bindingOptions.paperback} fieldName="binding_id" fieldValue={form.binding_id} onSelect={handleBindingSelect} isAvailable={(item) => availableBindings.includes(item.name)} stepAccessible={stepAccessibility.binding} />
          <OptionSection title="Hardcover Options" options={bindingOptions.hardcover} fieldName="binding_id" fieldValue={form.binding_id} onSelect={handleBindingSelect} isAvailable={(item) => availableBindings.includes(item.name)} stepAccessible={stepAccessibility.binding} />
          <SectionTitle>Interior Color</SectionTitle>
          <div className="flex gap-3 sm:gap-4 lg:gap-6 mb-8 sm:mb-10 flex-wrap">
            {OPTIONS_CONFIG.interiorColor.map((item, idx) => <OptionCard key={idx} item={item} fieldName="interior_color_id" fieldValue={form.interior_color_id} onSelect={(value) => setForm(prev => ({ ...prev, interior_color_id: value }))} isAvailable={isOptionAvailable('interiorColor', item.dbName)} stepAccessible={stepAccessibility.interiorColor} hasDbName={true} />)}
          </div>
          <SectionTitle>Paper Type</SectionTitle>
          <div className="flex gap-3 sm:gap-4 lg:gap-6 mb-8 sm:mb-10 flex-wrap">
            {OPTIONS_CONFIG.paperType.map((item, idx) => <OptionCard key={idx} item={item} fieldName="paper_type_id" fieldValue={form.paper_type_id} onSelect={(value) => setForm(prev => ({ ...prev, paper_type_id: value }))} isAvailable={isOptionAvailable('paperType', item.dbName)} stepAccessible={stepAccessibility.paperType} hasDbName={true} />)}
          </div>
          <SectionTitle>Cover Finish</SectionTitle>
          <div className="flex gap-3 sm:gap-4 lg:gap-6 mb-8 sm:mb-10 flex-wrap">
            {OPTIONS_CONFIG.coverFinish.map((item, idx) => <OptionCard key={idx} item={item} fieldName="cover_finish_id" fieldValue={form.cover_finish_id} onSelect={(value) => setForm(prev => ({ ...prev, cover_finish_id: value }))} isAvailable={isOptionAvailable('coverFinish', item.dbName)} stepAccessible={stepAccessibility.coverFinish} hasDbName={true} />)}
          </div>
          <div className="flex flex-col gap-4 p-4 bg-gradient-to-r from-[#016AB3] via-[#0096CD] to-[#00aedc] rounded-2xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="w-full sm:w-1/2"><p className="text-xs text-white opacity-90 mb-1">Quantity</p><FormInput name="quantity" value={form.quantity} onChange={handleChange} type="number" placeholder="Enter Quantity" min="1" className="h-12 bg-white text-black rounded-md" /></div>
              <div className="w-full sm:w-1/2 flex items-end"><button onClick={handleSubmit} className="w-full h-12 bg-[#F8C20A] hover:bg-yellow-500 text-black font-semibold rounded-md transition-colors">Calculate Price</button></div>
            </div>
            ...
            {result && (
              <div className="bg-white/20 rounded-lg p-4 text-white">
                ...
                <div className="space-y-1 text-sm">
                  ...
                  {result.discount > 0 && (<><div className="flex justify-between text-green-200"><span>Discount ({result.discountPercent}%):</span><span>-${result.discount}</span></div><div className="flex justify-between font-semibold text-yellow-200"><span>Final Price:</span><span>${result.finalPrice}</span></div></>)}
        ...
        <div className="w-full xl:w-2/5">
          <div className="sticky top-8 bg-white p-4 sm:p-6 lg:p-8 rounded-2xl shadow-lg">
            ...
            {[[['Book Size', form.bookSize || '-'], ['Page Count', form.page_count || '-']], [['Binding Type', form.binding_id || '-'], ['Interior Color', form.interior_color_id || '-']], [['Paper Type', form.paper_type_id || '-'], ['Cover Finish', form.cover_finish_id || '-']]].map((row, i) => (<React.Fragment key={i}><SummaryRow pairs={row} /><div className={`w-full h-px bg-gray-200 ${i === 2 ? 'my-4' : 'my-2'}`}></div></React.Fragment>))}
            <div className="flex justify-center mt-6"><div className="w-full max-w-xs"><RedirectButton /></div></div>
          </div>
        </div>
      </div>
      <Footer />
