...
const Shop = () => {
  ...
  const fetchShippingRate = async () => {
    const token = getToken();
    if (!token) {
      alert("You need to be logged in to calculate shipping. Redirecting to login...");
      navigate("/login");
      return;
    }
  ...
  const inputClass = "w-full p-2 md:p-3 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500";
  const buttonClass = "w-full py-2 md:py-3 text-white font-medium text-sm rounded-full shadow-md hover:shadow-lg transition-all duration-200";
  ...
  return (
    <>
      <Header />
      <div className="w-full h-[51px] flex items-center px-4 md:px-6" style={{ background: "linear-gradient(90deg, #016AB3 16.41%, #0096CD 60.03%, #00AEDC 87.93%)" }}>
      ...
      <div className="w-full min-h-screen bg-gradient-to-br from-[#eef4ff] to-[#fef6fb] px-4 md:px-6 py-6 md:py-10 font-sans relative">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-6 md:gap-10 relative">
          ...
          <div className="w-full lg:w-[60%] bg-gradient-to-br from-[#f2f9ff] via-white to-[#fff0f5] rounded-xl md:rounded-2xl shadow-xl p-4 md:p-6 lg:p-8">
            <h2 className="text-xl md:text-2xl font-bold text-[#2A428C] mb-4">Enter Your Shipping Address</h2>
            ...
            <div className="mb-4 md:mb-6 p-3 md:p-4 bg-blue-50 rounded-lg border border-blue-200">
              <h3 className="text-base md:text-lg font-semibold text-[#2A428C] mb-2 md:mb-3">Account Type</h3>
              <div className="flex flex-col md:flex-row gap-3 md:gap-6">
                {["individual", "enterprise"].map(type => (
                  <label key={type} className="flex items-center">
                    ...
                    <span className="text-sm md:text-base font-medium">{type === "enterprise" ? "Enterprise/Business" : "Individual"}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            {form.account_type === "enterprise" && (
              <div className="mb-4 md:mb-6 p-3 md:p-4 bg-green-50 rounded-lg border border-green-200">
                <label className="flex items-center">
                  ...
                  <span className="text-sm md:text-base font-medium text-green-800">I have a valid resale certificate (tax exempt)</span>
                </label>
              </div>
            )}
            ...
            <div className="flex flex-col md:flex-row gap-4 md:gap-6 mb-4">
              <input type="text" name="first_name" value={form.first_name} onChange={handleInputChange} placeholder="First Name" className={`${inputClass} md:w-1/2`} />
              <input type="text" name="last_name" value={form.last_name} onChange={handleInputChange} placeholder="Last Name" className={`${inputClass} md:w-1/2`} />
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 md:gap-6 mb-4">
              <input type="text" name="country" value={form.country} onChange={handleInputChange} placeholder="Country (e.g., US)" className={`${inputClass} md:w-1/2`} />
              <input type="text" name="state" value={form.state} onChange={handleInputChange} placeholder="State (e.g., TX)" className={`${inputClass} md:w-1/2`} />
            </div>

            <div className="flex flex-col md:flex-row gap-4 md:gap-6 mb-4">
              <input type="text" name="city" value={form.city} onChange={handleInputChange} placeholder="City" className={`${inputClass} md:w-1/2`} />
              <input type="text" name="postal_code" value={form.postal_code} onChange={handleInputChange} placeholder="Postal Code" className={`${inputClass} md:w-1/2`} />
            </div>
            ...
            {/* Error Message */}
            {shippingError && (
              <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4 text-sm md:text-base">{shippingError}</div>
            )}
            ...
            {availableServices && availableServices.length > 0 && (
              <div className="mb-4 p-3 md:p-4 bg-blue-50 rounded-lg border border-blue-200">
                <h3 className="text-base md:text-lg font-semibold text-[#2A428C] mb-2 md:mb-3">Available Shipping Services</h3>
                <div className="space-y-2">
                  {availableServices.map((service, index) => (
                    <label key={index} className="flex items-center p-2 md:p-3 border rounded-lg hover:bg-blue-100 cursor-pointer">
                      <input type="radio" name="selected_service" value={index}
                        ...
                        onChange={() => handleServiceSelection(service)} className="mr-2 md:mr-3" />
                      <div className="flex-1">
                        <div className="flex flex-col md:flex-row md:justify-between md:items-center">
                          <span className="font-medium text-sm md:text-base">{service.courier_name}</span>
                          <span className="font-bold text-[#2A428C] text-sm md:text-base">${service.total_charge.toFixed(2)}</span>
                        </div>
            ...
            <div className="w-full bg-gradient-to-br from-[#e0f3ff] via-white to-[#ffe4ec] rounded-xl md:rounded-2xl shadow-xl p-4 md:p-6">
              ...
              <div className="bg-[#E5FBFF] rounded-xl p-3 md:p-4 flex gap-3 md:gap-4 mb-4 md:mb-6">
                <div className="flex flex-col justify-center">
                  <h4 className="text-[#2A428C] font-bold text-lg md:text-xl mb-1">Book</h4>
