...
import { User, MapPin, Calendar, Search, Users, Grid, List, Eye, Mail, Clock, CreditCard, Filter } from "lucide-react";
...
const UserInfo = () => {
  ...
  const filteredProfiles = profiles.filter(profile =>
    ...
    `${profile.first_name} ${profile.last_name}`.toLowerCase().includes(search.toLowerCase())
  );
  ...
  const getAccountTypeBadge = (type) => {
    const styles = {
      ...
      standard: "bg-gradient-to-r from-gray-500 to-gray-600 text-white shadow-lg"
    };
    return styles[type?.toLowerCase()] || "bg-gradient-to-r from-gray-500 to-gray-600 text-white shadow-lg";
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 flex items-center justify-center p-4">
        <div className="text-center space-y-6">
          ...
          <div className="space-y-3">
            <h3 className="text-2xl font-bold text-gray-700">Loading Profiles</h3>
            <p className="text-gray-500 max-w-md">Fetching user data from the server. This won't take long...</p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col lg:flex-row items-center justify-between space-y-4 lg:space-y-0">
            <div className="text-center lg:text-left">
              ...
              <p className="text-gray-600 mt-2">Manage and explore user accounts</p>
                  ...
                  >
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          <h3 className="font-bold text-gray-800 text-lg">{profile.first_name} {profile.last_name}</h3>
                          <p className="text-gray-500 text-sm">@{profile.username}</p>
                        </div>
                      </div>
                      <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getAccountTypeBadge(profile.account_type)}`}>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center space-x-3 text-gray-600">
                        ...
                        <span className="text-sm truncate">{profile.city}, {profile.country}</span>
                      </div>
                      <div className="flex items-center space-x-3 text-gray-600">
                    ...
                    <tbody className="divide-y divide-gray-200/50">
                      {filteredProfiles.map((profile, index) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <div className="text-sm text-gray-800 font-medium">{profile.email}</div>
                          </td>
                          <td className="px-6 py-4">
                            <div className="text-sm text-gray-800">{profile.address}</div>
                            <div className="text-sm text-gray-500">{profile.city}, {profile.country} - {profile.postal_code}</div>
                          </td>
                          <td className="px-6 py-4">
                            <span className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold ${getAccountTypeBadge(profile.account_type)}`}>
                              {profile.account_type}
                            </span>
                          </td>
                          <td className="px-6 py-4">
                            <div className="text-sm text-gray-800">Created: {new Date(profile.created_at).toLocaleDateString()}</div>
                            <div className="text-sm text-gray-500">Updated: {new Date(profile.updated_at).toLocaleDateString()}</div>
                          </td>
                          <td className="px-6 py-4">
                            <button
                              onClick={() => setSelectedProfile(profile)}
            ...
            <div className="bg-white rounded-3xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl animate-scaleIn">
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <h2 className="text-3xl font-bold text-gray-800">{selectedProfile.first_name} {selectedProfile.last_name}</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-green-50 rounded-2xl">
                    ...
                    <p className="text-gray-600">{selectedProfile.city}, {selectedProfile.country} - {selectedProfile.postal_code}</p>
                  </div>
                  ...
                  <div className="p-4 bg-purple-50 rounded-2xl">
                    ...
                    <span className={`inline-flex px-4 py-2 rounded-full text-sm font-semibold ${getAccountTypeBadge(selectedProfile.account_type)}`}>
                ...
                <div className="space-y-4">
                  <div className="p-4 bg-yellow-50 rounded-2xl">
                    ...
                    <p className="text-gray-800">{new Date(selectedProfile.created_at).toLocaleString()}</p>
                  </div>
                  ...
                  <div className="p-4 bg-pink-50 rounded-2xl">
                    ...
                    <p className="text-gray-800">{new Date(selectedProfile.updated_at).toLocaleString()}</p>
                  </div>
