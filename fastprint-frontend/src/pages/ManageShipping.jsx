...
const ManageShipping = () => {
  ...
  const getUserName = (address) => {
    ...
    // Check for different possible name fields in the address JSON
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <div className="mb-6">
        ...
        {/* Debug information */}
        <div className="mt-4 p-4 bg-blue-100 border border-blue-400 rounded hidden"> {/* Add 'hidden' class to hide debug info in production */}
          <h3 className="font-bold">Debug Information:</h3>
          <pre className="whitespace-pre-wrap">{debugInfo}</pre>
          <div className="mt-2">
                                ...
                                {matchingBooks.map((book) => (
                                  <div key={book.id} className="bg-white p-4 rounded-lg shadow">
                                    ...
                                    <div className="mt-2 text-sm text-gray-600">
                                      <p><span className="font-medium">User Email:</span> {book.user_email}</p>
                                      <p><span className="font-medium">Category:</span> {book.category}</p>
                                      <p><span className="font-medium">Language:</span> {book.language}</p>
                                      <p><span className="font-medium">Page Count:</span> {book.page_count}</p>
                                      <p><span className="font-medium">Binding:</span> {book.binding_type}</p>
                                      <p><span className="font-medium">Paper:</span> {book.paper_type}</p>
                                      <p><span className="font-medium">Trim Size:</span> {book.trim_size}</p>
                                      {book.cover_description && (
                                        <p><span className="font-medium">Cover Description:</span> {book.cover_description}</p>
                                      )}
                                      <p><span className="font-medium">Uploaded:</span> {formatDate(book.created_at)}</p>
                                    </div>
                                  </div>
                                ))}
                              </div>
