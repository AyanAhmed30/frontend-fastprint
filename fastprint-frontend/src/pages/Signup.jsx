...
import LoginImage from '../assets/images/left-gradient.png';
...
const Signup = () => {
  ...
  // Password validation message as bullet points
  ...
  const handleSubmit = async (e) => {
    ...
    const nameError = !form.name ? 'Name is required.' : validateName(form.name) ? '' : 'Name must be at least 3 characters.';
  ...
  // Helper to render password validation as bullet list
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row overflow-hidden bg-white">
      ...
      <div
        className={`w-full lg:w-1/2 relative flex items-center justify-center h-56 sm:h-72 md:h-96 lg:h-auto transition-all duration-1000 ease-out ${mounted ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0'}`}
      ...
      >
        ...
        {/* Logo with animation (kept in place) */}
        <div
          className={`absolute top-4 left-4 sm:left-6 z-30 transition-all duration-700 delay-300 ${mounted ? 'scale-100 opacity-100' : 'scale-75 opacity-0'}`}
        >
          <img
            src={Logo}
            alt="Logo"
      ...
      <div
        className={`w-full lg:w-1/2 flex flex-col justify-center px-4 sm:px-8 md:px-12 py-8 transition-all duration-1000 ease-out relative ${mounted ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}
        ...
        <div className="relative z-10 w-full">
          ...
          >
            ...
            <div className="relative">
              <div
                className={`h-[3px] rounded-full transition-all duration-1000 delay-500 ${mounted ? 'w-[100px] sm:w-[150px] md:w-[180px]' : 'w-0'}`}
                style={{ backgroundColor: 'rgba(1, 106, 179, 1)' }}
              ></div>
              <div
                className={`mt-2 h-[2px] rounded-full mx-auto transition-all duration-1000 delay-700 ${mounted ? 'w-[50px] sm:w-[75px] md:w-[90px]' : 'w-0'}`}
                style={{ backgroundColor: 'rgba(1, 106, 179, 1)' }}
              ></div>
            </div>
          </div>
          ...
          {/* Form */}
          <div
            className={`max-w-xs sm:max-w-sm md:max-w-md mx-auto w-full transition-all duration-800 delay-500 ${mounted ? 'translate-y-0 opacity-100' : 'translate-y-6 opacity-0'}`}
          >
            <h2 className="text-base sm:text-lg md:text-xl font-semibold text-gray-800 mb-8 text-center">Create Your Account</h2>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col space-y-6" noValidate>
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  className={`w-full h-12 rounded-xl border-2 px-4 text-base transition-all duration-300 bg-white/70 backdrop-blur-sm
                    ${focusedField === 'name' || form.name ? 'border-blue-400 shadow-lg shadow-blue-100 scale-[1.02]' : 'border-gray-200 hover:border-gray-300'} 
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  disabled={formDisabled}
                  className={`w-full h-12 rounded-xl border-2 px-4 text-base transition-all duration-300 bg-white/70 backdrop-blur-sm
                    ${focusedField === 'email' || form.email ? 'border-blue-400 shadow-lg shadow-blue-100 scale-[1.02]' : 'border-gray-200 hover:border-gray-300'} 
                    ${formDisabled ? 'opacity-50 cursor-not-allowed' : 'hover:shadow-md'}
                    focus:outline-none focus:ring-0`}
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  disabled={formDisabled}
                  className={`w-full h-12 rounded-xl border-2 px-4 text-base transition-all duration-300 bg-white/70 backdrop-blur-sm
                    ${focusedField === 'password' || form.password ? 'border-blue-400 shadow-lg shadow-blue-100 scale-[1.02]' : 'border-gray-200 hover:border-gray-300'} 
                    ${formDisabled ? 'opacity-50 cursor-not-allowed' : 'hover:shadow-md'}
                    focus:outline-none focus:ring-0`}
              ...
              <button
                ...
                className={`relative h-12 rounded-full text-base font-medium text-white overflow-hidden transition-all duration-300 transform
                  ${formDisabled || !isFormValid() ? 'scale-95 opacity-80 cursor-not-allowed' : 'hover:scale-105 hover:shadow-xl hover:shadow-blue-200 active:scale-95'}
                `}
            ...
            {message && (
              <div
                className={`mt-6 p-3 border rounded-lg transition-all duration-300 ${
                  isSuccess ? 'bg-green-50 border-green-200 animate-bounce-gentle' : 'bg-red-50 border-red-200 animate-shake'
                }`}
              >
                <p className={`text-sm text-center flex items-center justify-center ${isSuccess ? 'text-green-600' : 'text-red-600'}`}>
                  {isSuccess ? (
